...
/* =========================
   CREATE SERVICE HELPER WITH VALIDATION
========================= */

async function createService(serviceData, masterFields, points) {
  
  // 🔥 VALIDATION: If reminderDays exists, dueDay must also exist
  if (serviceData.reminderDays && serviceData.reminderDays.length > 0) {
    if (!serviceData.dueDay || serviceData.dueDay.length === 0) {
      throw new Error(`❌ Service "${serviceData.name}" has reminderDays but no dueDay! Both must exist together.`);
    }
  }
  ...
  await prisma.serviceInputField.createMany({
    data: defaultFields.map((field, index) => ({
      ...
      required: field.required,
      serviceId: service.serviceId,
      masterFieldId: masterFields[index].masterFieldId
    }))
  });
  ...
  console.log(`  ✅ Created service: ${serviceData.name}`);
  if (serviceData.dueDay) {
    console.log(`     📅 Due days: [${serviceData.dueDay.join(', ')}]`);
  }
  if (serviceData.reminderDays) {
    console.log(`     🔔 Reminders: [${serviceData.reminderDays.join(', ')}] days before`);
  }
  return service;
}
...
async function main() {
  ...
  /* =========================
     CREATE ONE RECURRING SERVICE
  ========================= */
  console.log("Creating recurring service...");
  ...
  await createService({
    ...
    duration: "12",
    durationUnit: "MONTH",
    dueDay: [11, 20],  // ✅ Due on 11th and 20th of every month
    reminderDays: [5, 3, 2, 1, 0],  // ✅ Remind 5,3,2,1 days before and on due date
    individualPrice: "9999",
    offerPrice: "7999",
    isGstApplicable: "true",
    gstPercentage: "18",
    finalIndividualPrice: "9438",
    subCategoryId: gstFilingsSub.subCategoryId,
    employeeId: "cmneotly10001tz9ohgfy51pe",
    documentsRequired: "false"
  }, masterFields, servicePoints.gstReturnFiling);
